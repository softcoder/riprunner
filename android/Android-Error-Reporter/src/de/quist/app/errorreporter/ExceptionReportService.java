...
public class ExceptionReportService extends ReportingIntentService {
	...
	static final String EXTRA_TARGET_ERROR_URL = ExceptionReportService.class.getPackage().getName().concat(".extraTargetURL");
	...
	private void sendReport(Intent intent) throws UnsupportedEncodingException, NameNotFoundException {
		...
		Uri server = null;
		if(intent.hasExtra(ExceptionReportService.EXTRA_TARGET_ERROR_URL)) {
			server = getTargetUrl(intent.getStringExtra(ExceptionReportService.EXTRA_TARGET_ERROR_URL));
		}
		else {
			server = getTargetUrl(null);
		}
		...
		Log.d(TAG, "Created post request to server [" + server.toString() + " ] params [" + params + "]");
	...
	private Uri getTargetUrl(String overrideURL) throws NameNotFoundException {
		
		Log.i(TAG, "In getURL target URL registered as: [" + (overrideURL != null ? overrideURL : "null") + "]");
		
		if(overrideURL != null && 
				overrideURL.isEmpty() == false) {
