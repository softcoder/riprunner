...
//        gcm = null;
    ...
    private void setupCalloutUI(String respondingUserId) {
    	...
    	if(lastCallout != null &&
                !FireHallCalloutStatus.isComplete(lastCallout.getStatus())) {
     ...
     * shared preferences.
     */
    private void respondInBackground(final FireHallCalloutStatus statusType) {
        new AsyncTask<Void, Void, String>() {
		    ...
		    public void onClick(DialogInterface dialog, int whichButton) {
		        new AsyncTask<Void, Void, String>() {
		            ...
		            protected String doInBackground(Void... params) {
		                if (checkPlayServices()) {
		                	if(isLoggedIn()) {
		                		respondInBackground(FireHallCalloutStatus.Cancelled);
		                	}
		                } 
		                else {
		                    Log.i(Utils.TAG, Utils.getLineNumber() + ": No valid Google Play Services APK found.");
		    ...
		    public void onClick(DialogInterface dialog, int whichButton) {
		        new AsyncTask<Void, Void, String>() {
		            ...
		            protected String doInBackground(Void... params) {
		                if (checkPlayServices()) {
		                	if(isLoggedIn()) {
		                		respondInBackground(FireHallCalloutStatus.Complete);
		                	}
		                } 
		                else {
		                    Log.i(Utils.TAG, Utils.getLineNumber() + ": No valid Google Play Services APK found.");
	...
	void handleRespondClick() {
		new AsyncTask<Void, Void, String>() {
		    ...
		    protected String doInBackground(Void... params) {
		        if (checkPlayServices()) {
		        	if(isLoggedIn()) {
		        		respondInBackground(FireHallCalloutStatus.Responding);
		        	}
		        } 
		        else {
		            Log.i(Utils.TAG, Utils.getLineNumber() + ": No valid Google Play Services APK found.");
    ...
    private void sendResponseToBackend(FireHallAuthentication auth,
    		final FireHallCalloutStatus statusType) throws IOException {
    	...
    	params.put("status", String.valueOf(statusType.valueOf()));
...
//			        		});
       	...
       	if(!result.equals("") && result.startsWith("CALLOUT_ENDED=") &&
        	...
        	processCalloutResponseTrigger("Callout has ended!",
					...
					String.valueOf(FireHallCalloutStatus.Complete.valueOf()),
					null);
       	}
    }
	...
	public boolean onPrepareOptionsMenu (Menu menu) {
        ...
        if(logout_menu != null) logout_menu.setEnabled(isLoggedIn()); // here pass the index of save menu item
