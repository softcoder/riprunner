...
- It's very flexible. You are able to change many aspects of the behaviour and display of this application. You can select from various backend database engines (Mysql, MS SQL, Postgres, Oracle, etc). You can customize the user interface by making small overrides via thw 'twig' framework or completely repalce the user interface with your own. You can easily add on trigger mechanisms, support addiitional SMS providers all with relative ease.
