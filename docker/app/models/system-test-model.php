...
class SystemTestModel extends BaseViewModel {
    
	protected function getVarContainerName() { 
		return "systemtest_vm";
	}
	
	public function __get($name) {
	    if('callout_type_list' === $name) {
	...
	private function getTypeList() {
	    ...
	    foreach($rows as $row) {
	        ...
	        if($typeDef->isActiveForDate($current_date_time)) {
    	        $row['typeDef'] = $typeDef;
    	        $resultArray[] = $row;
	        }
	    }
