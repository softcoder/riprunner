...
RR_USER="riprunner"
RR_PASS="riprunner"
echo "=> Creating MySQL $RR_USER user with default password"

mysql -uroot -e "CREATE USER '$RR_USER'@'%' IDENTIFIED BY '$RR_PASS'"
mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO '$RR_USER'@'%' WITH GRANT OPTION"
...
echo "    mysql -u$RR_USER -p$RR_PASS -h<host> -P<port>"
echo ""
echo "Please remember to change the above password as soon as possible!"
echo "MySQL user 'root' has no password but only allows local connections"
echo "MySQL user '$RR_USER' has the default password and only allows local connections"
echo ""
