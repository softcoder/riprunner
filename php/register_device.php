...
if(isset($registration_id) && isset($firehall_id) && isset($user_id) && isset($user_pwd)) {
	...
	if($FIREHALL != null) {
		...
		if( $user_authenticated == true) {
			...
			if($FIREHALL->MOBILE->MOBILE_SIGNAL_ENABLED == false ||
				$FIREHALL->MOBILE->GCM_SIGNAL_ENABLED == false) {
				$log->error("Mobile and/or GCM is NOT enabled for this site for user_id [$user_id]");
				echo 'ERROR DISABLED!';
			}
			else {
