...
// 	global $log;
// 	$log->warn("START ==> Google App Engine email trigger for client [" . \riprunner\Authentication::getClientIPInfo() ."]");
...
// 				$log->warn("Email trigger checking #1 from [$from]");
				
// 				$from = extractDelimitedValueFromString($from, '~\<(.*?)\>~i', 1);
				
// 				$log->warn("Email trigger checking #2 from [$from]");
