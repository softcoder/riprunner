...
class PlancakeEmailParser {
    ...
    private function extractHeadersAndRawBody()
    {
        ...
        foreach ($lines as $line)
        {
            ...
            if ($this->isLineStartingWithPrintableChar($line)) // start of new header
            {
                ...
                sysLog(LOG_INFO,'#1 PARSE HEADER name['.$newHeader.'] value ['.$value.']');
            }
            else // more lines related to the current header
            {
                if ($currentHeader) { // to prevent notice from empty lines
                    ...
                    sysLog(LOG_INFO,'#2 PARSE HEADER name['.$currentHeader.'] value ['.substr($line, 1).']');
                }
            }
            $i++;
        }
     ...
     *
     * @return string
     * @throws Exception if a sender header is not found or if there are no sender
     */
    public function getSender()
    {
        ...
        return explode(',', $this->rawFields['sender']);
    }
